    ...
    implements EventList<E>
{
    ...
    /**
     * Create a new Set to EventList adapter for the specified set.
     *
     * @param set set, must not be null
     */
    SetEventListAdapter(final Set<E> set)
    {
        if (set == null)
        {
        ...
        this.set = set;
        eventList = GlazedLists.eventList(this.set);
    }
    ...
    {
        if (set.size() > eventList.size())
        {
            addToEventList();
        }
        else if (set.size() < eventList.size())
        {
            removeFromEventList();
        }
        else
    ...
    /**
     * Check contents of both the set and event list.
     */
