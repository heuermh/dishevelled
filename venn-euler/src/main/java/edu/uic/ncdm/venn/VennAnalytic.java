...
//	private static final CyLogger logger = CyLogger.getLogger(VennAnalytic.class);
	...
	private VennDiagram collectResults() {
		...
		List<Set<String>> luneIds = new ArrayList<Set<String>>(nPolygons -1);
		...
		for (int i = 1; i < nPolygons; i++) {
			...
			char[] c = encode(i);
			//System.out.println("lune i: " + i + " encoded:  " + java.util.Arrays.toString(c));
			String s = "";
			...
			for (int j = 0; j < c.length; j++) {
				if (c[j] == '1') {
					...
					//System.out.println("luneCenter : " + (i-1) + " adding center: " + j);
					numIntersecting += 1.0;
...
//		logger.info("stress = " + stress + ", stress01 = " + stress01 + ", stress05 = " + stress05);
		...
		return new VennDiagram(centers, diameters, polyAreas, residuals, circleLabels, residualLabels, 
		                       colors, intersectionCounts, warnings, luneCenters, luneIds, stress, stress01, stress05);
	}
